...
countLinks = (content) => {
  ...
  markdownLinkExtractor(content, true).forEach((link) => {
    if (
      link.href.startsWith("/") || // remove internal links
      ...
      link.href === "https://rknight.me"
    )
      return; // remove direct links to my site
...
module.exports = {
  ...
  eleventyComputed: {
    ...
    data: (data) => {
      ...
      let breadcrumbParts = [];
      const CHEVRON =
        '<svg class="breadcrumb-divider"><use xlink:href="#chevron-right"></use></svg>';
      if (pageIndex) {
        ...
        while (parts.length > 0) {
          ...
          if (!pageData) break;
          breadcrumbPartsHTML.unshift(
            `<a href="${breadcrumbUrl}">${pageData.title}</a>`
          );
